...
class PostLikesView(APIView):
    ...
    def get(self, request, pk):
        publication = Post.objects.get(id=pk)
...
class PublicationsView(ModelViewSet):
    ...
    queryset = Post.objects.prefetch_related('post_images', 'post_comments').annotate(
    ...
    def get_object(self):
        obj = Post.objects.prefetch_related('post_images').annotate(
            owner_nick_name=F('owner__username'),
            owner_avatar=F('owner__avatar'),
            likes_count=Count('post_likes')
